...
contract MetadataRenderer is IMetadataRenderer, MetadataRenderAdminCheck {
        ...
        requireSenderAdmin(target)
    {
        ...
        emit DescriptionUpdated({target: target, tokenId: tokenId, sender: msg.sender, newDescription: newDescription});
    }
    ...
    function updateName(address target, uint256 tokenId, string memory newName) external requireSenderAdmin(target) {
        tokenInfos[target].name = newName;

        emit NameUpdated({target: target, tokenId: tokenId, sender: msg.sender, newName: newName});
    }
    ...
    function initializeWithData(bytes memory data, uint256 tokenId) external {
        ...
        (string memory name, string memory description, string memory imageURI, string memory animationURI) =
    ...
    function contractURI() external view override returns (string memory) {
        ...
        if (bytes(tokenInfos[target].imageURI).length > 0) {
            imageSpace = abi.encodePacked('", "image": "', tokenInfos[target].imageURI);
        }
        return string(
