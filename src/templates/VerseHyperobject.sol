...
abstract contract VerseHyperobject is IVerseHyperobject {
    ...
    /// @notice The address of the paired exchange contract
    ...
    /// @dev internal token id counter, increments upon each mint
    ...
    constructor(string memory _name, string memory _symbol, address _exchange) {
        name = _name;
        symbol = _symbol;
        exchange = _exchange;
        nextId++;
    ...
    /// @notice Mint function which ensures exchange contract is only authorized minter
    /// @dev Should not be overriden by inheriting contract
    function mint(address _recipient) external {
      if (msg.sender != exchange) revert Unauthorized();
      _mint(_recipient, nextId++);
    }
